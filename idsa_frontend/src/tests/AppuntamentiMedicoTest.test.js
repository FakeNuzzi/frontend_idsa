...
/*
test('navigates to scriviRisultato', async () => {
  const appuntamenti = [
    { id_app: 1, id_paziente: 1, id_medico: 1, id_visita: 1, id_slot: 1 },
    { id_app: 2, id_paziente: 1, id_medico: 1, id_visita: 1, id_slot: 2 },
    { id_app: 3, id_paziente: 1, id_medico: 1, id_visita: 1, id_slot: 3 }
  ...
  const risultatoButtons = await screen.findAllByText(/Risultato/i);
  
  expect(risultatoButtons).toHaveLength(appuntamenti.length);
  
  console.log(risultatoButtons[0]);

  await act(async () => {
    fireEvent.click(risultatoButtons[0]);
  });

  expect(mockNavigator).toHaveBeenCalledWith('/risultatoForm/1/1');
